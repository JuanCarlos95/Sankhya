...
DELETE FROM TGFCAB WHERE NUNOTA IN (SELECT NUNOTA FROM NOTAS_GERADAS);


SELECT CEIL((J.MTZ / 100)) AS MTZ, MTZ2, (J.MTZ / 100) AS PROVAMTZ, J.MTZ,
       CEIL((J.OFC / 100)) AS OFC, OFC2, (J.OFC / 100) AS PROVAOFC, J.OFC,
       CEIL((J.RG / 100)) AS RG, RG2,    (J.RG / 100)  AS PROVARG, J.RG
  FROM (
SELECT (SELECT COUNT(CODPROD) FROM IMPORT_ESTOQUE WHERE CODEMP = 1) AS MTZ,
       (SELECT COUNT(NUNOTA) FROM TGFCAB WHERE TRUNC(DTNEG) = TRUNC(SYSDATE) AND CODEMP = 1 AND NUNOTA > 10) AS MTZ2,
       (SELECT COUNT(CODPROD) FROM IMPORT_ESTOQUE WHERE CODEMP = 2) AS RG,
       (SELECT COUNT(NUNOTA) FROM TGFCAB WHERE TRUNC(DTNEG) = TRUNC(SYSDATE) AND CODEMP = 2 AND NUNOTA > 10) AS RG2,
       (SELECT COUNT(CODPROD) FROM IMPORT_ESTOQUE WHERE CODEMP = 3) AS OFC,
       (SELECT COUNT(NUNOTA) FROM TGFCAB WHERE TRUNC(DTNEG) = TRUNC(SYSDATE) AND CODEMP = 3 AND NUNOTA > 10) AS OFC2
  FROM DUAL) J;